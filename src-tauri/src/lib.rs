...
mod utils;
use utils::{get_mod_latest_version, get_mod_manifest};
...
#[tauri::command]
async fn check_mods(directory: String) -> Result<Vec<Mod>, String> {
    // 读取目录下的所有文件夹下面的 manifest.json 文件
    let mut mods: Vec<Mod> = Vec::new();
    for entry in std::fs::read_dir(directory).unwrap() {
        let result = get_mod_manifest(entry.unwrap().path().to_str().unwrap())
            .await
            .unwrap();
